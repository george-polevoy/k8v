...
export const MIN_CONNECTION_STROKE_WIDTH = 0.25;
export const MAX_CONNECTION_STROKE_WIDTH = 24;
export const MIN_CONNECTION_BRIGHTNESS_DELTA = 24;
export const CONNECTION_BRIGHTNESS_ADJUSTMENT = 42;

export const DEFAULT_GRAPH_CONNECTION_STROKE = Object.freeze({
...
function adjustHexBrightness(color, delta) {
...
function resolveBrightness(color) {
  const r = Number.parseInt(color.slice(1, 3), 16);
  const g = Number.parseInt(color.slice(3, 5), 16);
  const b = Number.parseInt(color.slice(5, 7), 16);
...
function normalizeBackgroundColor(foregroundColor, requestedBackgroundColor) {
...
export function normalizeGraphConnectionStroke(value) {
