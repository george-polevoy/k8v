...
const GraphQueryNodeFieldSchema = z.enum([
  'id',
  'name',
  'type',
  'version',
...
type GraphQueryNodeField = z.infer<typeof GraphQueryNodeFieldSchema>;

const GraphQueryConnectionFieldSchema = z.enum([
  'id',
  'sourceNodeId',
  'sourcePort',
  'targetNodeId',
...
type GraphQueryConnectionField = z.infer<typeof GraphQueryConnectionFieldSchema>;
...
type GraphQueryRequest = z.infer<typeof GraphQueryRequestSchema>;
...
export function createApp(deps?: AppDependencies) {
  ...
  app.post('/api/graphs/:id/query', validate(GraphQueryRequestSchema), async (req, res) => {
    try {
      ...
      const query = req.body as GraphQueryRequest;
      ...
      if (unknownNodeIds.length > 0) {
        return res.status(400).json({
          error: `Start node ids not found in graph ${graph.id}: ${unknownNodeIds.join(', ')}`,
        });
      }
