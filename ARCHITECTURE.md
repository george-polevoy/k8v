...
- Uses an internal `RecomputeStateStore` to track node execution state snapshots, node/result change cursors, and sparse runtime deltas exposed through graph runtime-state responses and SSE-triggered refreshes
...
- Maintains a latest-result projection per graph/node for cheap snapshot and delta runtime-state reads without rescanning full result history
...
- Builds graph-scoped runtime-state snapshots and cursor-based deltas returned by `GET /api/graphs/:id/runtime-state` and command responses
...
- Additional runtime: `PythonProcessRuntime` (`python_process`) backed by a graph/env-scoped warm Python service and per-env worker pool
...
- Sizes warm Python workers from graph `recomputeConcurrency`, preserving concurrent Python recompute within the same graph/env scope
- Reloads cwd-local pure-Python helper modules between warm executions while keeping installed packages and native extensions warm
...
- Adds a dedicated transient wasm invocation tool (`algo_injection_run`) instead of overloading `bulk_edit` for binary/module management
...
- Publishes dedicated wasm-invocation docs/examples that describe the fixed wasm host API (`graph_get`, `graph_query`, staged `bulk_edit`)
...
#### Wasm Algo Service (`packages/backend/src/core/WasmAlgoService.ts`)
- Accepts transient wasm invocation requests that point to an absolute filesystem path readable by the backend host
- Validates each wasm module at invocation time against the fixed JSON ABI (`memory`, `alloc`, `run` by default) and an allowlisted import namespace
...
- Real-time graph/runtime sync through SSE plus cursor-based runtime-state delta polling fallback
...
5. **Recomputation**: Backend recompute manager detects impacted chains, schedules queued work, and publishes runtime updates consumed through SSE and graph-scoped runtime-state snapshots/deltas
